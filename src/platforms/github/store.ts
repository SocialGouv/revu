...
export default class GithubStore implements PlatformStore {
  ...
  constructor(
    ...
    getRepositoryAndBranch(): {
        ...
        return {
            ...
            branch: (this.octokitContext.payload as (PullRequestOpenedPayload | ReviewRequestedPayload)).pull_request.head.ref
        }
    }

  async createPlatformContext(): Promise<PlatformContext> {
    const payload = this.octokitContext.payload as ReviewRequestedPayload | ReadyForReviewPayload
  ...
  isPRCreatedByBot(): boolean {
    const payload = this.octokitContext.payload as PullRequestOpenedPayload | ReadyForReviewPayload;
