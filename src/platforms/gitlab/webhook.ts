...
export class GitlabWebhookApp implements WebhookApp {
  ...
  constructor() {
    ...
    r.post('/', async (req, res) => {
      ...
      if (
        req.headers['X-Gitlab-Event'] === undefined ||
        !ALLOWED_EVENTS.includes(req.headers['X-Gitlab-Event'] as string)
      ) {
  ...
  async mergeRequestOpened(_payload: MergeRequestEvent) {}

  async handleReviewRequested(_payload: CommentEvent) {}
}
